...
class TradingController extends BaseController {
  ...
  async executeTrade(req, res) {
    const { contractAddress, tokenAddress, seller, buyer, amount, price } = req.body;
    
    // 验证参数
    if (!this.validateRequired(res, { contractAddress, tokenAddress, seller, buyer, amount, price })) {
      return;
    }
    ...
    await this.handleContractAction(
      ...
      async () => {
        // 获取合约实例
        const contract = ContractUtils.getContract('TradingManager', contractAddress);
  ...
  async batchExecuteTrade(req, res) {
    const { contractAddress, trades } = req.body;
    
    // 验证参数
    if (!this.validateRequired(res, { contractAddress, trades })) {
      return;
    }
