...
export const toggleEmergencyMode = async (req:, res:) => {
  try {
    ...
    // 获取当前状态
    const currentMode = await (connectedRoleManager ).emergencyMode();
...
export const toggleTradingPause = async (req:, res:) => {
  try {
    ...
    // 获取当前状态
    const currentPaused = await (connectedTradingManager ).paused();
 ...
 * 获取合约地址
 */
export const getContractAddresses = async (req:, res:) => {
  try {
    // 从环境配置中获取合约地址
