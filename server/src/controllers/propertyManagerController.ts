...
export const getPropertyInfo = async (req: Request, res: Response) => {
  try {
    ...
    const connectedToken = await createContractFromAddress(
      tokenAddress,
      (await getContract('PropertyToken')).interface,
    );
...
export const updatePropertyInfo = async (req: Request, res: Response) => {
  try {
    ...
    // 使用私钥创建已连接的合约实例
    const connectedPropertyManager = await getContractWithPrivateKey(
      'PropertyManager', 
      managerPrivateKey
    );
