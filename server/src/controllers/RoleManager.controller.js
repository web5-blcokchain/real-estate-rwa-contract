...
// 读取ABI文件
const abiPath = path.resolve(process.cwd(), 'config/abi/RoleManager.json');
...
if (fs.existsSync(addressConfigPath)) {
  ...
  contractAddress = addressConfig[networkType]?.RoleManager;
}

// 如果配置文件中没有，尝试从环境变量获取
if (!contractAddress) {
  contractAddress = process.env.CONTRACT_ROLEMANAGER_ADDRESS;
}
