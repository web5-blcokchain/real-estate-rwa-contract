...
/**
 * @title RewardManager - 日本房地产收益与分配管理合约
 * @author Fashi Shijian团队
 * @notice 本合约负责管理房产资产的收益分配，包括租金、分红和奖励的收集与分发
 * 
 * @dev 合约功能描述：
 * 1. 收益分配管理：支持创建、管理和执行不同类型的收益分配（分红、租金、奖金）
 * 2. 默克尔树分配：采用默克尔树技术高效处理大规模分配，降低gas成本
 * 3. 多币种支持：支持多种稳定币作为分配媒介
 * 4. 收益提取：代币持有者可以根据持有比例提取相应收益
 * 5. 紧急提款：在特殊情况下支持紧急资金提取
 * 6. 状态管理：追踪每个分配的状态（待分配、进行中、已完成、已取消）
 * 
 * @dev 技术特点：
 * - 默克尔树验证：使用密码学证明高效验证用户的分配权益
 * - 优化存储结构：使用紧凑数据类型(uint8, uint40)节省存储成本
 * - 灵活的分配机制：支持基于时间窗口的分配计划
 * - 防重放保护：记录已提取历史防止重复提取
 * 
 * @dev 与其他模块的关联：
 * - RealEstateSystem：依赖核心系统合约进行权限验证
 * - PropertyManager：基于房产信息和代币分布计算分配
 * - PropertyToken：获取代币持有信息，确定分配比例
 * - TradingManager：与交易系统协作，支持基于交易量的奖励机制
 * 
 * @dev 权限控制：
 * - ADMIN：可创建分配计划、添加支持的稳定币、授权合约、处理紧急情况
 * - MANAGER：可更新分配状态、管理分配进度、设置分配参数
 * - OPERATOR：可查询分配状态、协助用户解决提取问题
 * 
 * @dev 业务流程：
 * 1. 创建分配：ADMIN或MANAGER创建分配计划，设置总金额、类型和时间窗口
 * 2. 计算分配：使用默克尔树计算每个用户的分配份额
 * 3. 开始分配：激活分配，允许用户开始提取
 * 4. 用户提取：用户提供默克尔证明验证自己的份额并提取收益
 * 5. 完成分配：所有资金分配完毕或时间窗口结束后，完成分配
 * 
 * @dev 安全考虑：
